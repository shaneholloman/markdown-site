...
- **Modified** `src/App.tsx`: Added `AgentReadyWidget` and `UpdateBanner` from `@waynesutton/agent-ready/react`. Widget renders floating bottom-right with dark theme, gated on `VITE_CONVEX_SITE_URL`.
...
- **New file** `agent-ready.config.json`: Component config with app name, URL, analytics, Claude AI descriptions, and `sitemapEnabled: false`.
